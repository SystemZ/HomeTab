...
func GmailGetThreads(token string) *gmail.ListThreadsResponse {
	srv := GmailAuth(token)
	r, err := srv.Users.Threads.List("me").Do()
	if err != nil {
		log.Printf("%v", err)
	}
	return r
...
func GmailGetThread(token string, threadId string) *gmail.Thread {
	srv := GmailAuth(token)
	r, err := srv.Users.Threads.Get("me", threadId).Format("full").Do()
	if err != nil {
		log.Printf("%v", err)
	}
	return r
